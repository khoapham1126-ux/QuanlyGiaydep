...
public class InventoryDAO {
    ...
    public boolean insert(Inventory i) {
        String sql = "INSERT INTO Inventory(ProductID, Quantity, MinQuantity, LastUpdated) VALUES (?, ?, ?, GETDATE())";
    ...
    public boolean update(Inventory i) {
        String sql = "UPDATE Inventory SET ProductID = ?, Quantity = ?, MinQuantity = ?, LastUpdated = GETDATE() WHERE InventoryID = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, i.getProductId());
            ps.setInt(2, i.getQuantity());
